...
To build the final jar and optimize the rlcrm application for production, run:
...
Performance tests are run by [Gatling][] and written in Scala. They're located in [src/test/java/gatling/simulations](core-service-impl/src/test/java/gatling/simulations).
...
Note: we have turned off forced authentication redirect for UI in [src/main/docker/sonar.yml](core-service-impl/src/main/docker/sonar.yml) for out of the box experience while trying out SonarQube, for real use cases turn it back on.
...
You can use Docker to improve your JHipster development experience. A number of docker-compose configuration are available in the [src/main/docker](core-service-impl/src/main/docker) folder to launch required third party services.
